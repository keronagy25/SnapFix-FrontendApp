  ...
  return (
    <TouchableOpacity onPress={onPress} activeOpacity={0.75} style={styles.tabItem}>
      <Animated.View style={{ alignItems: "center", transform: [{ translateY }] }}>
        ...
        <Text style={{ fontFamily: focused ? "Poppins_600SemiBold" : "Poppins_400Regular", fontSize: 9.5, color: focused ? C.accent : C.muted, letterSpacing: focused ? 0.4 : 0, marginTop: 1 }}>
          {label}
        </Text>
...
const styles = StyleSheet.create({
  ...
  iconPill:    { position: "absolute", width: 48, height: 34, borderRadius: 14, backgroundColor: "rgba(30,58,138,0.09)" },
  dot:         { marginTop: 5, width: 4, height: 4, borderRadius: 2, backgroundColor: C.cyan, shadowColor: C.cyan, shadowOffset: { width: 0, height: 0 }, shadowOpacity: 1, shadowRadius: 4 },
});
...
export default function CustomerLayout() {
  return (
    ...
    >
      ...
      <Tabs.Screen name="booking" />   {/* folder → shows booking/index.tsx */}
      <Tabs.Screen name="wallet"  />
      <Tabs.Screen name="chat"    />   {/* folder → shows chat/index.tsx    */}
      <Tabs.Screen name="profile" />
      {/* standalone screens hidden from tab bar */}
