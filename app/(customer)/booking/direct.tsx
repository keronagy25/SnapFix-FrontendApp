...
import { createDirectBooking, type DirectBookingPayload } from "@/services/bookingService";
...
function ProviderPickerModal({ visible, providers, selectedId, onSelect, onClose, loading }: {
  visible: boolean; providers: FavoriteProvider[]; selectedId: string; onSelect: (provider: FavoriteProvider) => void; onClose: () => void; loading: boolean;
}) {
    ...
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose} statusBarTranslucent>
      <View style={{ flex: 1, backgroundColor: "rgba(0,0,0,0.5)", justifyContent: "flex-end" }}>
        ...
        <View style={{ backgroundColor: "#fff", borderTopLeftRadius: 28, borderTopRightRadius: 28, padding: 20, maxHeight: "80%", paddingBottom: Platform.OS === "ios" ? 40 : 24 }}>
          ...
          <Text style={{ fontFamily: Typography.fonts.bold, fontSize: 17, color: "#0F172A", marginBottom: 14 }}>Select Favorite Provider</Text>
          ...
          {loading ? (
            ...
            <View style={{ alignItems: "center", paddingVertical: 40 }}>
              ...
              <Text style={{ fontFamily: Typography.fonts.medium, fontSize: 14, color: "#94A3B8", marginTop: 12, textAlign: "center" }}>
                No favorite providers yet
              </Text>
              <Text style={{ fontFamily: Typography.fonts.regular, fontSize: 12, color: "#CBD5E1", marginTop: 4, textAlign: "center" }}>
                Add providers to favorites from booking history
              </Text>
            </View>
          ) : (
            <ScrollView showsVerticalScrollIndicator={false}>
              {providers.map((provider) => (
                <TouchableOpacity
                  key={provider.id}
                  onPress={() => { onSelect(provider); onClose(); }}
                  style={{
                    paddingVertical: 14,
                    borderBottomWidth: 1,
                ...
                >
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    {provider.business_name && (
                      <Text style={{ fontFamily: Typography.fonts.regular, fontSize: 12, color: "#64748B" }}>{provider.business_name}</Text>
                    )}
                    <View style={{ flexDirection: "row", alignItems: "center", gap: 8, marginTop: 4 }}>
                      <View style={{ flexDirection: "row", alignItems: "center", gap: 2 }}>
                        <Star size={12} color="#F59E0B" fill="#F59E0B" />
                        <Text style={{ fontFamily: Typography.fonts.semibold, fontSize: 12, color: "#0F172A" }}>
                          {provider.rating?.toFixed(1) ?? "—"}
                        </Text>
                      </View>
                      <View style={{ flexDirection: "row", alignItems: "center", gap: 4 }}>
                        <View style={{ width: 6, height: 6, borderRadius: 3, backgroundColor: provider.is_available ? "#10B981" : "#94A3B8" }} />
                        <Text style={{ fontFamily: Typography.fonts.regular, fontSize: 11, color: provider.is_available ? "#10B981" : "#94A3B8" }}>
                          {provider.is_available ? "Available" : "Unavailable"}
                        </Text>
                      </View>
                    </View>
                  </View>
                  {selectedId === provider.id && <CheckCircle size={20} color="#1E3A8A" />}
                </TouchableOpacity>
              ))}
            </ScrollView>
          )}
...
// Location Picker Modal - Same as your create screen
...
export default function DirectBookingScreen() {
  ...
  const params = useLocalSearchParams<{ provider_id?: string; provider_name?: string }>();
  ...
  useEffect(() => {
    (async () => {
      setLoadingData(true);
      try {
        const [cats, regs, favs] = await Promise.all([
        ...
        setRegions(regs);
        setFavorites(favs);
        if (params.provider_id && favs.find(f => f.id === params.provider_id)) {
          const p = favs.find(f => f.id === params.provider_id);
          set("provider_name", `${p?.first_name} ${p?.last_name}`);
        }
      } catch {
        setFeedback({ ok: false, title: "Failed to Load", msg: "Could not load form data." });
      } finally {
  ...
  const validate = (): boolean => {
    ...
    if (!form.provider_id) e.provider = "Please select a favorite provider";
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const payload: DirectBookingPayload = {
      ...
      console.log("Submitting direct booking for provider:", form.provider_id);
      const response = await createDirectBooking(payload, token, photos);
      console.log("Direct booking response:", response);
      setSuccess(true);
    } catch (err: any) {
      const d = err?.data ?? {};
      console.log("[DirectBooking] error:", JSON.stringify(d, null, 2));
      
      const fieldMap: Record<string, string> = {
        provider_id: "provider", category: "category", region: "region",
      ...
      if (hasInline) {
        ...
        const msg = getApiError(err, "Failed to create direct booking.");
        setApiError(msg);
  ...
  if (success) {
    return (
      <View style={{ flex: 1, backgroundColor: "#F8FAFC", alignItems: "center", justifyContent: "center", paddingHorizontal: 32 }}>
        ...
        <Text style={{ fontFamily: Typography.fonts.extrabold, fontSize: 24, color: "#0F172A", marginBottom: 10, textAlign: "center" }}>Direct Booking Submitted!</Text>
        <Text style={{ fontFamily: Typography.fonts.regular, fontSize: 14, color: "#64748B", textAlign: "center", lineHeight: 22, marginBottom: 32 }}>
          Your request has been directly assigned to {form.provider_name}.{"\n"}They will review and respond shortly.
        </Text>
      ...
      <View style={{ flex: 1, backgroundColor: "#F8FAFC" }}>
        ...
        <LinearGradient colors={["#1E3A8A", "#1E40AF"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }}
          ...
          <View style={{ flexDirection: "row", alignItems: "center", gap: 14 }}>
            ...
            <Text style={{ fontFamily: Typography.fonts.extrabold, fontSize: 22, color: "#fff" }}>Direct Booking</Text>
          </View>
        </LinearGradient>
  ...
  const selectedProvider = favorites.find(p => p.id === form.provider_id);
    ...
    <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={{ flex: 1, backgroundColor: "#F8FAFC" }}>
      ...
      <LinearGradient colors={["#1E3A8A", "#1E40AF"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }}
        ...
        <View style={{ flexDirection: "row", alignItems: "center", gap: 14 }}>
          ...
          <View>
            <Text style={{ fontFamily: Typography.fonts.extrabold, fontSize: 22, color: "#fff" }}>Direct Booking</Text>
            <Text style={{ fontFamily: Typography.fonts.regular, fontSize: 13, color: "rgba(255,255,255,0.6)" }}>Book your favorite provider directly</Text>
          </View>
        </View>
      </LinearGradient>
      ...
      >
        ...
        {/* Provider Selection Section - NEW for direct booking */}
        <View>
          <View style={{ backgroundColor: "#fff", borderRadius: 20, padding: 16, marginBottom: 14, borderWidth: 1, borderColor: "#F1F5F9", shadowColor: "#1E3A8A", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 8, elevation: 2 }}>
            <Text style={{ fontFamily: Typography.fonts.bold, fontSize: 15, color: "#0F172A", marginBottom: 14 }}>
              ❤️ Select Favorite Provider <Text style={{ color: "#EF4444" }}>*</Text>
            </Text>
            <Field label="Provider" required error={errors.provider}>
              <SelectPill 
                selected={form.provider_name || (selectedProvider ? `${selectedProvider.first_name} ${selectedProvider.last_name}` : "")} 
                onPress={() => setProviderPicker(true)} 
                placeholder="Choose a provider from your favorites" 
                hasError={!!errors.provider} 
              />
            </Field>
            {selectedProvider && (
              <View style={{ marginTop: 12, padding: 12, backgroundColor: "#EFF6FF", borderRadius: 12, flexDirection: "row", alignItems: "center", gap: 12 }}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <View style={{ flexDirection: "row", alignItems: "center", gap: 8, marginTop: 4 }}>
                    <View style={{ flexDirection: "row", alignItems: "center", gap: 2 }}>
                      <Star size={12} color="#F59E0B" fill="#F59E0B" />
                      <Text style={{ fontSize: 12 }}>{selectedProvider.rating?.toFixed(1) ?? "—"}</Text>
                    </View>
                    <View style={{ flexDirection: "row", alignItems: "center", gap: 4 }}>
                      <View style={{ width: 6, height: 6, borderRadius: 3, backgroundColor: selectedProvider.is_available ? "#10B981" : "#94A3B8" }} />
                      <Text style={{ fontSize: 11, color: selectedProvider.is_available ? "#10B981" : "#94A3B8" }}>
                        {selectedProvider.is_available ? "Available now" : "Currently unavailable"}
                      </Text>
                    </View>
                  </View>
                </View>
            ...
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 12 }}>
              {photos.map((photo, index) => (
                <View key={index} style={{ position: 'relative' }}>
                  ...
                  <TouchableOpacity onPress={() => removePhoto(index)} style={{ position: 'absolute', top: -8, right: -8, backgroundColor: '#EF4444', borderRadius: 12, padding: 4 }}>
                    <X size={16} color="#fff" />
                  </TouchableOpacity>
                </View>
              ))}
              
              <TouchableOpacity onPress={pickImages} style={{ width: 100, height: 100, borderRadius: 12, backgroundColor: "#F1F5F9", alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: "#E2E8F0", borderStyle: 'dashed' }}>
                <ImageIcon size={24} color="#64748B" />
                <Text style={{ fontFamily: Typography.fonts.regular, fontSize: 11, color: "#64748B", marginTop: 4 }}>Gallery</Text>
              </TouchableOpacity>
              
              <TouchableOpacity onPress={takePhoto} style={{ width: 100, height: 100, borderRadius: 12, backgroundColor: "#F1F5F9", alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: "#E2E8F0", borderStyle: 'dashed' }}>
                <Plus size={24} color="#64748B" />
                <Text style={{ fontFamily: Typography.fonts.regular, fontSize: 11, color: "#64748B", marginTop: 4 }}>Camera</Text>
              </TouchableOpacity>
            </View>
            
            {errors.photos && <Text style={{ fontFamily: Typography.fonts.regular, fontSize: 12, color: "#EF4444", marginTop: 12 }}>⚠ {errors.photos}</Text>}
            {photos.length > 0 && <Text style={{ fontFamily: Typography.fonts.regular, fontSize: 11, color: "#10B981", marginTop: 8 }}>✓ {photos.length} photo(s) selected</Text>}
          </View>
        </View>
        ...
        {/* LOCATION DETAILS with Map Picker */}
        <View>
          <View style={{ backgroundColor: "#fff", borderRadius: 20, padding: 16, marginBottom: 14, borderWidth: 1, borderColor: "#F1F5F9", shadowColor: "#1E3A8A", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 8, elevation: 2 }}>
        ...
        {/* BOOKING SUMMARY */}
        {(form.provider_name || form.category_name || form.region_name || form.title) && (
          <View>
            <View style={{ backgroundColor: "#EFF6FF", borderRadius: 18, padding: 16, marginBottom: 14, borderWidth: 1, borderColor: "#BFDBFE" }}>
              <Text style={{ fontFamily: Typography.fonts.semibold, fontSize: 12, color: "#1D4ED8", marginBottom: 10, letterSpacing: 0.5 }}>📋 DIRECT BOOKING SUMMARY</Text>
              {form.provider_name && <SRow label="Provider" value={`❤️ ${form.provider_name}`} />}
          ...
          <TouchableOpacity onPress={handleSubmit} disabled={submitting} activeOpacity={0.88}
            ...
            <LinearGradient colors={["#1E3A8A", "#1E40AF"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}
              ...
              {submitting
                ...
                : <><Heart size={18} color="#06B6D4" /><Text style={{ fontFamily: Typography.fonts.bold, fontSize: 16, color: "#fff" }}>Direct Book {form.provider_name ? `with ${form.provider_name}` : "Provider"}</Text></>
              }
            </LinearGradient>
          </TouchableOpacity>
        </View>
      ...
      {/* Pickers */}
      <ProviderPickerModal visible={providerPicker} providers={favorites} selectedId={form.provider_id} onSelect={(p) => { set("provider_id", p.id); set("provider_name", `${p.first_name} ${p.last_name}`); }} onClose={() => setProviderPicker(false)} loading={loadingFavorites} />
