...
export default function ProviderLoginScreen() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // Clear any existing onboarding token
      ...
      const status = profile?.verification_status ?? "approved";
      if (status === "approved" || status === "verified") {
        router.replace("/(provider)/dashboard" as any);
      } else if (status === "pending" || status === "under_review") {
        // Still under review - redirect to pending screen
        router.replace("/(auth)/provider/pending" as any);
      } else if (status === "rejected") {
        setFeedback({
          title: "Account Not Approved",
          msg: profile?.rejection_reason || "Your account was not approved. Please contact support."
        });
      ...
      if (nonField.toLowerCase().includes("still under review") || 
          nonField.toLowerCase().includes("pending")) {
        // User exists but not approved yet - redirect to pending
        setRole("provider");
        setUser({ role: "provider", email: email.trim(), verification_status: "pending" } as any);
        router.replace("/(auth)/provider/pending" as any);
        return;
      }

      // Check if registration incomplete
      if (nonField.toLowerCase().includes("complete your registration")) {
        setRole("provider");
        setUser({ role: "provider", email: email.trim(), verification_status: "draft" } as any);
        router.replace("/(auth)/provider/onboarding/personal" as any);
        return;
      }
