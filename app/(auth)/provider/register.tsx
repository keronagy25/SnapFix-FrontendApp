...
import { User, Mail, Phone, Lock, ArrowLeft, Eye, EyeOff, CheckCircle, AlertCircle } from "@/components/ui/lucide-icon";
...
export default function ProviderRegisterScreen() {
  ...
  const validate = () => {
    ...
    if (!/^01[0125][0-9]{8}$/.test(form.phone)) e.phone = "Must be 11 digits starting with 010, 011, 012, or 015";
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      // Store the onboarding_token for subsequent onboarding calls
      if (res.onboarding_token) {
        setOnboardingToken(res.onboarding_token);
      }
  ...
  return (
    <View style={{ flex: 1, backgroundColor: "#F8FAFC" }}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" contentContainerStyle={{ padding: 24, paddingBottom: 60 }}>
        <FieldRow label="First Name" value={form.first_name} onChange={set("first_name")} placeholder="e.g. Ahmed" icon={User} capitalize="words" error={errors.first_name} />
        <FieldRow label="Last Name" value={form.last_name} onChange={set("last_name")} placeholder="e.g. Hassan" icon={User} capitalize="words" error={errors.last_name} />
        ...
        <FieldRow label="Phone Number" value={form.phone} onChange={set("phone")} placeholder="01xxxxxxxxx" icon={Phone} keyboard="phone-pad" error={errors.phone} hint="11 digits starting with 010, 011, 012, or 015" maxLen={11} />
        <FieldRow label="Password" value={form.password} onChange={set("password")} placeholder="Minimum 8 characters" icon={Lock} secure={!showPass} error={errors.password}
          showToggle onToggle={() => setShowPass(v => !v)} hint="At least 8 characters" />
        ...
        <TouchableOpacity onPress={handleRegister} disabled={loading} activeOpacity={0.85}
          ...
          <LinearGradient colors={["#06B6D4", "#0284C7"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}
            ...
            {loading ? <ActivityIndicator size="small" color="#fff" />
              : <Text style={{ fontFamily: Typography.fonts.bold, fontSize: 16, color: "#fff" }}>Create Account & Continue</Text>}
          </LinearGradient>
        </TouchableOpacity>
